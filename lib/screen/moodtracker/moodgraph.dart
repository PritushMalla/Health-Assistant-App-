        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            scoresByDate.forEach((date, scores) {
              double averageScore = scores.reduce((a, b) => a + b) /
                  scores.length; // Calculate average score
              spots.add(FlSpot(spots.length.toDouble(),
                  averageScore)); // Add a spot for this day
            ...
            if (spots.isEmpty) {
              return Center(child: Text('No valid Health data to display.'));
            }

            return LineChart(LineChartData(
              gridData: FlGridData(
                  show: false), // Disable grid lines for a cleaner look
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        if (value % 1 == 0) {
                          // Adjust the interval (show whole numbers only)
                          return Text(
              ...
              minX: 0,
              maxX: (spots.length - 1)
                  .toDouble(), // Adjust according to number of data points
              minY: 0,
              ...
              lineBarsData: [
                LineChartBarData(
                  ...
                  // Color of the line
                  belowBarData:
                      BarAreaData(show: false), // Disable area below the line
                ...
                touchTooltipData: LineTouchTooltipData(
                  ...
                  getTooltipItems: (touchedSpots) {
                    return touchedSpots.map((touchedSpot) {
                      ...
                      final date = dates[
                          index]; // Get the corresponding date from the index
                      return LineTooltipItem(
                        date, // Display the date in the tooltip
                        const TextStyle(
                            color: Colors.white), // Text style for the tooltip
                      );
                    }).toList();
                  },
                ),
